...
- [x] SNTP 墙钟同步——Wi-Fi 上线后从 Cloudflare anycast 拉取时间，并以单调递增钟作为底，保证时间戳不会回跳
...
| ⏳  | 睡眠定时 / 定时静音                                  | 2 天    | SNTP 已提供可靠墙钟，这件事现在可行。增加一个可选的倒计时（N 分钟后静音 + LCD 调暗）以及可选的每日定时开 / 关，状态持久化在 `storage` 分区。面向贡献者开放 —— 工程量多在 UI 面（Web 控制台开关、LCD 浮层），固件运行时管道几乎不动。 |
...
- BLE LE Audio source (Auracast / LC3) —— 芯片硬件支持，但 `esp-
  ...
  估。
- BLE HID 遥控 —— 已被交付的 LAN Web 控制台覆盖：配对成本高、无屏
